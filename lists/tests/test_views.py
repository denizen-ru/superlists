...
class ListViewTest(TestCase):
    ...
    def test_displays_all_items(self):
        ...
        Item.objects.create(text='other list itemey 1', list=other_list_)
        Item.objects.create(text='other list itemey 2', list=other_list_)

        response = self.client.get('/lists/%d/' % (correct_list_.id,))
        self.assertContains(response, 'itemey 1')
        self.assertContains(response, 'itemey 2')
        self.assertNotContains(response, 'other list itemey 1')
        self.assertNotContains(response, 'other list itemey 2')

    @skip('where is the source ?!')
    ...
    def test_POST_redirects_to_list_view(self):
        ...
        response = self.client.post('/lists/%d/' % (correct_list.id,), data={
                                    'item_text': 'A new item for the existing list'})
        self.assertRedirects(response, '/lists/%d/' % (correct_list.id,))

    def test_validation_errors_end_up_onlists_page(self):
        list_ = List.objects.create()
